...
export function DietGenerator({ data }: { data: DietData }) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4">
      <Card className="w-full max-w-4xl border-0 p-5 md:p-6">
        ...
        {output && (
          <div className="bg-card rounded-lg p-2 border border-border max-h-[500px] overflow-y-auto">
            <div className="prose prose-sm max-w-none">
              <ReactMarkdown
                components={{
                  h2: ({ node, ...props }) => (
                    <h2
                      className="text-xl font-black text-green-600 my-1"
                      {...props}
                    />
                  ),
                  h1: ({ node, ...props }) => (
